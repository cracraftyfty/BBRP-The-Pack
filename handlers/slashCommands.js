        ...
        readdirSync("./slashCommands/").forEach((dir) => {
			if(lstatSync(`./slashCommands/${dir}`).isDirectory()) {
				...
				if(cmdSetup && cmdSetup.Folder) {
								...
								if(pull.options && pull.options.length > 0){
									for(const option of pull.options){
										if(option.User && option.User.name && option.User.description){
											...
											console.log(`[ThePack] A Option is missing the Name or/and the Description of ${pull.name}`)
										}
									}
								}
								return subcommand;
					...
					return console.log(`[ThePack] The Subcommand-Folder ${dir} is not in the dirSetup Configuration!`);
				}
			} else {
		...
		client.on("ready", () => {
			if(config.loadSlashsGlobal){
				...
				.then(slashCommandsData => {
					console.log(`[ThePack] ${slashCommandsData.size} slashCommands ${`(With ${slashCommandsData.map(d => d.options).flat().length} Subcommands)`.green} Loaded for all: ${`All possible Guilds`.underline}`.brightGreen); 
					console.log(`[ThePack] Because u are Using Global Settings, it can take up to 1 hour until the Commands are changed!`.bold.yellow)
				...
				client.guilds.cache.map(g => g).forEach((guild) => {
					try{
						...
						.then(slashCommandsData => {
							console.log(`[ThePack] ${slashCommandsData.size} slashCommands ${`(With ${slashCommandsData.map(d => d.options).flat().length} Subcommands)`.green} Loaded for: ${`${guild.name}`.underline}`.brightGreen); 
						}).catch((e)=>console.log(e));
					}catch (e){
