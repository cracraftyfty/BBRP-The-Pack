...
module.exports = {
    name: "remove",
    description: "Removes mentioned item from the storage",
    cooldown: 5,
    memberpermissions: [],
    ...
    run: async (client, interaction) => {
        ...
        if(!INVENTORY_FILE[type][storageID].storage.hasOwnProperty(item)) return interaction.reply({
            embeds: [
                new MessageEmbed()
                .setColor('RED')
                .setDescription(`${settings.emotes.wrong} **${item}** does not exist in ${interaction.channel}`)
            ],
            ephemeral: true
        })
        else {
            for(let keys in INVENTORY_FILE[type][storageID].storage) {
                ...
                if(INVENTORY_FILE[type][storageID].storage[keys] <= 0) delete INVENTORY_FILE[type][storageID].storage[keys]
        ...
        interaction.reply({
            embeds: [
                ...
                .setDescription(`${settings.emotes.check}  **${nwc(amount)}x ${cap(item)}** successfully removed from inventory: ${interaction.channel}`)
